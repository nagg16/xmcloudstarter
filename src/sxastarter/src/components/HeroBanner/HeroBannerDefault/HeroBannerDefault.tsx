...
import Video from '@/core/atoms/Video';
import EpicHeader from '@/core/molecules/EpicHeader/EpicHeader';

export const HeroBannerDefault = (props: HeroBannerProps): JSX.Element => {
  ...
  return (
    <div className={`relative w-full ${params?.styles || ''} ${rendering?.componentName || ''}`}>
      <div className="relative flex min-h-screen items-center justify-center ">
        ...
        {fields.video?.jsonValue && (
          <Video field={fields.video?.jsonValue} className="size-full object-cover" />
        )}

        {fields.image?.jsonValue && fields.video?.jsonValue && (
          <div className="absolute inset-0 size-full">
            {fields.image && (
              <Image field={fields.image?.jsonValue} className="size-full object-cover" />
            )}
...
export default HeroBannerDefault;
